...
class HotkeysManager:
    ...
    def __on_press(self, key):
        ...
        if self.changeKey == False and self.main_app.prevent_record == False:
            ...
            if keyPressed in self.hotkey_detection:
            ...
            if (
                ...
                self.main_app.after(0, lambda: self.macro.start_record(by_hotkey))
                ...
                self.main_app.after(0, self.macro.stop_record)
                    ...
                    and self.macro.record == False
                    and self.macro.playback == False
                    and (self.main_app.macro_recorded == True or self.main_app.loop_scripts_var.get())
            ):
                self.main_app.after(0, self.main_app.on_play_click)
                ...
                self.main_app.after(0, lambda: self.macro.stop_playback(by_hotkey))
    ...
    def __on_release(self, key):
        if self.changeKey == False and self.main_app.prevent_record == False:
            ...
            try:
                self.hotkey_detection.remove(keyPressed)
            except ValueError:
                pass
