...
class UserSettings:
    ...
    def check_new_options(self):
        ...
        if "Last_Script_Folder" not in userSettings["Others"]:
            userSettings["Others"]["Last_Script_Folder"] = ""
        ...
        if "Image_Recognition" not in userSettings:
            userSettings["Image_Recognition"] = {
                "Enabled": False,
                "Image_Path": "",
                "Area": None,
                "Alarm": False,
        ...
        if "Special_Cases" not in userSettings:
            # Initialize with default for N, SP1, SP2, SP3, SP4
            userSettings["Special_Cases"] = {
                "N": {
                    "Enabled": userSettings.get("Image_Recognition", {}).get("Enabled", False),
                    "Image_Path": userSettings.get("Image_Recognition", {}).get("Image_Path", ""),
                    "Area": None,
                    "Alarm": userSettings.get("Image_Recognition", {}).get("Alarm", False),
                    "Confidence": userSettings.get("Image_Recognition", {}).get("Confidence", 0.75),
                    "Loop_Limit": userSettings.get("Playback", {}).get("Loop_Limit", None)
                },
                "SP1": {
                    "Enabled": False, "Image_Path": "", "Area": None,
                    "Alarm": False, "Confidence": 0.75, "Loop_Limit": None, "Stop_Program": False
                },
                "SP2": {
                    "Enabled": False, "Image_Path": "", "Area": None,
                    "Alarm": False, "Confidence": 0.75, "Loop_Limit": None, "Stop_Program": False
        ...
        # Ensure all cases have all keys (including new SP3, SP4)
        for case in ["N", "SP1", "SP2", "SP3", "SP4", "SP5", "SP6"]:
            if case not in userSettings["Special_Cases"]:
                userSettings["Special_Cases"][case] = {
                    "Enabled": False, "Image_Path": "", "Area": None,
                    "Alarm": False, "Confidence": 0.75, "Loop_Limit": None, "Stop_Program": False,
                    "TG_Alert": False, "TG_Message": ""
                }
            else:
                c_dict = userSettings["Special_Cases"][case]
                if "Enabled" not in c_dict: c_dict["Enabled"] = False
                if "Image_Path" not in c_dict: c_dict["Image_Path"] = ""
                if "Area" not in c_dict: c_dict["Area"] = None
                if "Alarm" not in c_dict: c_dict["Alarm"] = False
                if "Confidence" not in c_dict: c_dict["Confidence"] = 0.75
                if "Loop_Limit" not in c_dict: c_dict["Loop_Limit"] = None
                if "Stop_Program" not in c_dict: c_dict["Stop_Program"] = False
                if "TG_Alert" not in c_dict: c_dict["TG_Alert"] = False
                if "TG_Message" not in c_dict: c_dict["TG_Message"] = ""
                # Migrate old single-area [x1,y1,x2,y2] → [[x1,y1,x2,y2]]
                area = c_dict.get("Area")
                if area is not None and isinstance(area, list) and len(area) == 4 and not isinstance(area[0], list):
                    c_dict["Area"] = [area]

        if "Loop_Scripts" not in userSettings["Playback"]:
            userSettings["Playback"]["Loop_Scripts"] = False
